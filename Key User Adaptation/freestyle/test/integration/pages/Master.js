			...
			actions: {
				...
				iOpenViewSettingsDialog: function() {
					return this.waitFor({
						...
						viewName: sViewName,
						check: function() {
							var oViewSettingsDialog = Opa5.getWindow().sap.ui.require("sap/ui/core/Element").getElementById("viewSettingsDialog");
			...
			assertions: {
				iShouldSeeTheBusyIndicator: function() {
					return this.waitFor({
						...
						success: function(oList) {
							...
							QUnit.assert.ok(oList.getBusy(), "The master list is busy");
						},
				...
				theListShouldContainAGroupHeader: function() {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function() {
							QUnit.assert.ok(true, "Master list is grouped");
						},
				...
				theListHeaderDisplaysZeroHits: function() {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "SEPMRA_C_PD_Product (0)"
						}),
						success: function() {
							QUnit.assert.ok(true, "The list header displays 'SEPMRA_C_PD_Product (0)'");
						},
				...
				theListHasEntries: function() {
					return this.waitFor({
						...
						success: function() {
							QUnit.assert.ok(true, "The list has items");
						},
				...
				theListShouldNotContainGroupHeaders: function() {
					...
					return this.waitFor({
						...
						success: function() {
							QUnit.assert.ok(true, "Master list does not contain a group header although grouping has been removed.");
						},
				...
				theListShouldBeSortedAscendingOnField: function(sField) {
					...
					return this.waitFor({
						...
						success: function() {
							QUnit.assert.ok(true, "Master list has been sorted correctly for field '" + sField + "'.");
						},
				...
				theListShouldBeFilteredOnUnitNumberValue: function(iThreshhold, bGreaterThan, oRange) {
					...
					return this.waitFor({
						...
						success: function() {
							QUnit.assert.ok(true, "Master list has been filtered correctly with filter value '" + iThreshhold + "'.");
						},
				...
				iShouldSeeTheList: function() {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function(oList) {
							QUnit.assert.ok(oList, "Found the object List");
						},
				...
				theListShouldHaveNEntries: function(iObjIndex) {
					return this.waitFor({
						...
						success: function(oList) {
							QUnit.assert.strictEqual(oList.getItems().length, iObjIndex, "The list has x items");
						},
				...
				theListShouldHaveAllEntries: function() {
					return this.waitFor({
						...
						success: function(oList) {
							QUnit.assert.strictEqual(oList.getItems().length, oList.getGrowingThreshold(), "The growing list has 10 items");
						},
				...
				iShouldSeeTheNoDataTextForNoSearchResults: function() {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function(oList) {
							QUnit.assert.strictEqual(oList.getNoDataText(), oList.getModel("i18n").getProperty("No matching products found"),
					...
					return this.waitFor({
						...
						success: function(oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "SEPMRA_C_PD_Product (" + iExpectedLength + ")"
								}),
								success: function() {
									QUnit.assert.ok(true, "The master page header displays " + iExpectedLength + " items");
								},
				...
				theFirstItemShouldBeSelected: function() {
					return this.waitFor({
						...
						success: function(oList) {
							QUnit.assert.strictEqual(oList.getItems()[0], oList.getSelectedItem(), "The first object is selected");
						},
				...
				theListShouldHaveNoSelection: function() {
					return this.waitFor({
						...
						success: function(oList) {
							QUnit.assert.strictEqual(oList.getSelectedItems().length, 0, "The list selection is removed");
						},
				...
				theRememberedListItemShouldBeSelected: function() {
					this.waitFor({
						...
						success: function(oSelectedItem) {
							QUnit.assert.strictEqual(oSelectedItem.getTitle(), this.getContext().currentListItem.getTitle(), "The list selection is incorrect");
						},
