			...
			actions: {
				...
				iOpenFilterDialog: function() {
					return this.waitFor({
						...
						viewName: "ProductMaster",
						check: function() {
							var oSortSettingsDialog = Opa5.getWindow().sap.ui.require("sap/ui/core/Element").getElementById("filterSettingsDialog");
				...
				OpenSortDialog: function() {
					return this.waitFor({
						...
						viewName: sViewName,
						check: function() {
							var oSortSettingsDialog = Opa5.getWindow().sap.ui.require("sap/ui/core/Element").getElementById("sortSettingsDialog");
							// check if the dialog is still open - wait until it is closed
							// view settings dialog has no is open function and no open close events so checking the domref is the only option here
							// if there is no view settings dialog yet, there is no need to wait
							return !oSortSettingsDialog || oSortSettingsDialog.$().length === 0;
				...
				iOpenGroupDialog: function() {
					return this.waitFor({
						...
						viewName: sViewName,
						check: function() {
							var oGroupSettingsDialog = Opa5.getWindow().sap.ui.require("sap/ui/core/Element").getElementById("groupingSettingsDialog");
							// check if the dialog is still open - wait until it is closed
							// view settings dialog has no is open function and no open close events so checking the domref is the only option here
							// if there is no view settings dialog yet, there is no need to wait
				...
				iCheckIfItemSearchedIsMatchingOnMasterList: function() {
					return this.waitFor({
						...
						success: function(oList) {
							...
							Opa5.assert.strictEqual(sFirstObjectTitleAfterSearch.trim(), sFirstObjectTitle.trim(), "Searched string is matching"); // eslint-disable-line no-undef
						}
					});
				},
				iSeeFilterBarStatus: function(status) {
					return this.waitFor({
						...
						success: function(oFilterStatus) {
							Opa5.assert.strictEqual(oFilterStatus.getText().trim(), status, "Filter label is shown"); // eslint-disable-line no-undef
						},
