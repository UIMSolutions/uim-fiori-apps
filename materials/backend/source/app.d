...
void main() {
    ...
    // auto customer_service = new KundenService();
    // router.get("/web/", &customer_service.zeigeDashboard);
    // router.get("/api/mehr-kunden", &customer_service.ladeMehrKunden);

    auto portal = new PortalController();
    router.get("/web/", &portal.index);
    router.get("/web/dashboard", &portal.dashboard);
